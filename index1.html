...
/* ── FLASH OVERLAY (visual alert backup) ── */
#flash{position:fixed;inset:0;z-index:9999;pointer-events:none;opacity:0;
background:rgba(184,92,56,.25);transition:opacity .15s;}
...
.toast-ico{font-size:20px;flex-shrink:0;line-height:1.2;}
.toast-body{}
.toast-title{font-size:13px;font-weight:800;line-height:1.3;}
.toast-sub{font-size:11px;opacity:.75;margin-top:2px;font-weight:500;}

/* ── HEADER ── */
.hdr{flex-shrink:0;padding:12px 18px 9px;
...
.hdr-clock{font-family:Georgia,serif;font-size:20px;font-weight:700;
color:var(–ink);letter-spacing:.5px;line-height:1;}
.hdr-date{font-size:10px;font-weight:600;color:var(–muted);margin-top:2px;letter-spacing:.3px;}
.hdr-bell{background:none;border:1px solid var(–bdr);border-radius:10px;
cursor:pointer;font-size:16px;padding:5px 9px;color:var(–ink2);
box-shadow:var(–sh0);transition:all .15s;line-height:1;}
.hdr-bell:active{background:var(–surf3);}
.hdr-bell.armed{border-color:var(–moss);background:var(–mosslt,#ecf2e2);color:var(–moss);}
...
/* ── ACTIVE BLOCK TICKER ── */
.ticker{flex-shrink:0;margin:0 16px 8px;border-radius:12px;
padding:10px 14px;display:none;align-items:center;gap:10px;
border:1.5px solid var(–terra);background:var(–teralt);
box-shadow:var(–sh0);}
.ticker.show{display:flex;}
.ticker-dot{width:8px;height:8px;border-radius:50%;background:var(–terra);
flex-shrink:0;animation:pulse 1.4s ease-in-out infinite;}
@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(1.3)}}
.ticker-txt{flex:1;font-size:12px;font-weight:700;color:var(–clay);}
.ticker-time{font-family:Georgia,serif;font-size:13px;font-weight:700;
...
.tblk.now{border-left-width:4px;box-shadow:var(–sh2);}
...
.tb-n{font-size:10px;font-weight:500;margin-top:3px;opacity:.58;white-space:nowrap;
...
border-radius:11px;font-size:13px;font-weight:600;cursor:pointer;position:relative;color:var(–ink);}
...
.inst-s{font-size:11px;color:rgba(255,255,255,.72);margin-top:1px;font-weight:500;}
...
.btn-d{padding:13px 14px;border-radius:14px;border:1.5px solid rgba(184,92,56,.25);
background:var(–teralt);color:var(–terra);font-family:inherit;font-size:14px;font-weight:700;cursor:pointer;}
...
<div class="toast" id="toast">
  ...
  <div class="toast-body">
    <div class="toast-title" id="tTitle">Alert</div>
...
<div id="app">
  ...
  <header class="hdr">
    ...
    <button class="hdr-bell" id="bellBtn" onclick="toggleAlerts()">🔔</button>
  </header>
  ...
  <div class="view on" id="day-view">
    ...
    <div class="ticker" id="ticker">
      ...
      <div class="ticker-txt" id="tickerTxt">Active block</div>
    ...
    <div class="inst-hero">
      <div class="inst-bar">
        <div class="inst-hd">
          ...
          <div><div class="inst-ttl">Install on iPhone via GitHub Pages</div>
               <div class="inst-s">Free · Offline after first open · Update anytime</div></div>
        </div>
        <div class="inst-steps">
          <div class="istep"><div class="in">1</div><div class="it">Go to <b>github.com</b> in Safari · sign up free · create Public repo named <b>focusflow</b></div></div>
          <div class="istep"><div class="in">2</div><div class="it">Tap <b>"uploading an existing file"</b> → pick <b>index.html</b> → <b>Commit changes</b></div></div>
          <div class="istep"><div class="in">3</div><div class="it">Tap <b>Settings → Pages → Source: main → Save</b></div></div>
          <div class="istep"><div class="in">4</div><div class="it">Open <b>yourusername.github.io/focusflow</b> → Share ⎋ → <b>Add to Home Screen</b></div></div>
          <div class="istep"><div class="in">↑</div><div class="it"><b>To update:</b> upload a new index.html to GitHub — app refreshes next open with internet</div></div>
        </div>
      </div>
      <div class="inst-note">Data saved <b>only on your iPhone</b> · Never uploaded</div>
    </div>

```
<div class="s-wrap">
  <div class="ss" style="margin-top:18px">
    <div class="shd">Alerts</div>
    <div class="scard">
      <div class="srow">
        <div><label>Alerts enabled</label><small>🔔 Bell icon = armed · tap to toggle</small></div>
        <button class="tog" id="togAlerts" onclick="toggleAlerts()"></button>
      </div>
      <div class="srow">
        <div><label>Start of block alert</label><small>Chime + vibration when block begins</small></div>
        <button class="tog on" id="togStart" onclick="togSet('alertStart',this)"></button>
      </div>
      <div class="srow">
        <div><label>End of block alert</label><small>Chime + vibration when block ends</small></div>
        <button class="tog on" id="togEnd" onclick="togSet('alertEnd',this)"></button>
      </div>
      <div class="srow" style="cursor:pointer" onclick="testAlert()">
        <label style="cursor:pointer">▶ Test alert now</label>
  ...
  <div class="ss">
    ...
    <div class="scard">
      <div class="srow">
        <div><label>Current time marker</label></div>
  ...
  <div class="foot">FocusFlow v3 · All data on your device only</div>
</div>
```
...
/* ═══════════════════════════════════════════
   FOCUSFLOW v3
   Alert engine: polling every 30s
   Audio: Web Audio API, unlocked on first tap
   Vibration: navigator.vibrate (Android+)
   ═══════════════════════════════════════════ */
...
var P={showNow:true,showHalf:true,defDur:30,alerts:true,alertStart:true,alertEnd:true};
var B={};
...
/* ── Play chime ── */
/*
  type 'start': ascending bright tone — signals GO
  type 'end'  : descending warm tone  — signals DONE
  Tone is audible even when ringer is on.
  On silent mode iOS mutes Web Audio — vibration fires instead.
*/
function playChime(type){
  ...
  try{
    ...
    function beep(freq, start, duration, vol){
      ...
      o.type = 'triangle';  /* softer than square, louder than sine */
      o.frequency.value = freq;
      g.gain.setValueAtTime(0, t+start);
      g.gain.linearRampToValueAtTime(vol||0.4, t+start+0.02);
      g.gain.exponentialRampToValueAtTime(0.001, t+start+duration);
      o.start(t+start);
      o.stop(t+start+duration+0.01);
    }

    if(type==='start'){
      /* E4 → G4 → B4 : bright rising triad */
      beep(330, 0.00, 0.30, 0.45);
      beep(392, 0.22, 0.35, 0.45);
      beep(494, 0.44, 0.60, 0.40);
    } else {
      /* B4 → G4 → E4 : calm descending triad */
      beep(494, 0.00, 0.30, 0.40);
      beep(392, 0.22, 0.35, 0.38);
      beep(330, 0.44, 0.70, 0.35);
    }
  }catch(e){}
}
...
function vibrate(type){
  ...
  if(type==='start') navigator.vibrate([80, 60, 80]);       /* two short pulses */
  else               navigator.vibrate([60, 40, 60, 40, 200]); /* done pattern */
}
...
/* ── Toast ── */
var _toastTmr = null;
function showToast(ico, title, sub, dur){
  ...
  _toastTmr = setTimeout(function(){ t.classList.remove('show'); }, dur||4500);
}

/* ═══════════════════════════════
   ALERT POLLING ENGINE
   Checks every 30 seconds.
   Compares HH:MM of NOW against
   each block's start and end time.
   Fires only once per block per event
   using a fired-set keyed to date.
   ═══════════════════════════════ */
var _firedAlerts = {}; /* key: "YYYY-MM-DD|blockId|start|end" */
var _lastPollMin = -1; /* avoid double-fire within same minute */

function alertKey(bId, type){ return todK()+'|'+bId+'|'+type; }
...
function pollAlerts(){
  ...
  if(nowMin === _lastPollMin) return;
  _lastPollMin = nowMin;

  var bl = getB(todK());
  bl.forEach(function(b){
    var parts = b.start.split(':');
    var bh = +parts[0], bm = +parts[1];
    ...
    if(P.alertStart && nowMin === startMin){
      var sk = alertKey(b.id,'start');
      if(!_firedAlerts[sk]){
        _firedAlerts[sk] = true;
        playChime('start');
        vibrate('start');
        flashScreen();
        showToast(cat.e,
          '▶ Starting: '+b.act,
          mStr(b.dur)+' block — focus time!', 5000);
      }
    }

    /* END alert */
    if(P.alertEnd && nowMin === endMin){
      var ek = alertKey(b.id,'end');
      if(!_firedAlerts[ek]){
        _firedAlerts[ek] = true;
        playChime('end');
        vibrate('end');
        flashScreen();
        showToast('✅',
          'Done: '+b.act,
          'Great work! Block complete ✓', 5000);
        renderDay(); /* refresh active-block highlight */
      }
    }
  });
}
...
function testAlert(){
  ...
  vibrate('start');
  flashScreen();
  showToast('🔔','Alert test','Sound + vibration + flash — all working!', 4000);
}

/* ── Toggle alerts on/off ── */
function toggleAlerts(){
  P.alerts = !P.alerts; saveP();
  updateBellUI();
}
function updateBellUI(){
  var b = document.getElementById('bellBtn');
  var t = document.getElementById('togAlerts');
  b.className = 'hdr-bell'+(P.alerts?' armed':'');
  b.textContent = P.alerts ? '🔔' : '🔕';
  if(t) t.className = 'tog'+(P.alerts?' on':'');
}
...
window.onload = function(){
  buildCsg(); buildScatg(); initSettings();
  tick();
  /* Poll every 30 seconds — precise enough, no drift */
  ...
  /* Also poll immediately to catch any block starting right now */
  maybeResetAlerts();
...
function tick(){
  var n = new Date();
  document.getElementById('hClock').textContent = p2(n.getHours())+':'+p2(n.getMinutes());
  document.getElementById('hDate').textContent = DS[n.getDay()]+' '+n.getDate()+' '+MS[n.getMonth()]+' '+n.getFullYear();
}
...
function renderDay(){
  ...
  /* Active block ticker */
  var nowMins = isT ? (now.getHours()-HRS)*60+now.getMinutes() : -1;
  ...
  if(isT){
    ...
    bl.forEach(function(b){
      ...
      var sm=bh*60+bm;
      if(sm<=nowTotalMin && nowTotalMin<sm+b.dur) activeBlock=b;
    });
  }
  var tk=document.getElementById('ticker');
  if(activeBlock){
    var cat=CATS.find(function(c){return c.id===activeBlock.cat;})||CATS[0];
    var pts=activeBlock.start.split(':'), bh=+pts[0], bm=+pts[1];
    var endM=(bh*60+bm+activeBlock.dur);
    var remMin=endM-(now.getHours()*60+now.getMinutes());
    document.getElementById('tickerTxt').textContent=cat.e+' '+activeBlock.act;
    document.getElementById('tickerTime').textContent=mStr(remMin)+' left';
  ...
  var nowTotalMin2=isT?now.getHours()*60+now.getMinutes():-1;
  bl.forEach(function(b){
    var pts=b.start.split(':'), bh=+pts[0], bm=+pts[1];
    var sm=(bh-HRS)*60+bm;
    ...
    var isNowBlock=isT&&(bh*60+bm<=nowTotalMin2)&&(nowTotalMin2<em);
    var el=document.createElement('div');
    el.className='tblk'+(isNowBlock?' now':'');
    el.style.cssText='top:'+sm*PPM+'px;height:'+hpx+'px;left:0;right:0;'+
      'background:'+cat.bg+';border-left-color:'+cat.color+';';
    el.innerHTML=
  ...
  setTimeout(function(){sc.scrollTop=isT?Math.max(0,(now.getHours()-HRS-.8)*60*PPM):0;},60);
}
...
function openAdd(){
  ...
  setTimeout(function(){document.getElementById('fAct').focus();},300);
}
...
function initSettings(){
  ...
  document.getElementById('togStart').className='tog'+(P.alertStart?' on':'');
  document.getElementById('togEnd').className='tog'+(P.alertEnd?' on':'');
  updateBellUI();
  buildScatg();
}
function buildScatg(){
...
function togSet(key,btn){
  P[key]=!P[key]; btn.className='tog'+(P[key]?' on':''); saveP();
  if(activeTab()==='day')renderDay();
}
function doExport(){
  try{
  ...
  }catch(e){alert('On iPhone: Share → Save to Files');}
}
function doClear(){
