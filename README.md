...
:root{
  ...
  --moss:#557040;--mosslt:#ecf2e2;
  --clay:#8a5630;
  --green:#3a7a40;--greenlt:#e8f4ea;
...
.hdr-t{font-size:10px;font-weight:600;color:var(–muted);letter-spacing:.3px;}
.notif-btn{
background:none;border:none;cursor:pointer;
font-size:18px;line-height:1;padding:2px 0 2px 10px;
...
.tblk.active-now{box-shadow:var(–sh2);border-left-width:4px;}
...
.tb-bell{position:absolute;top:7px;right:8px;font-size:10px;opacity:.5;}
...
.notif-card{
background:linear-gradient(135deg,var(–moss) 0%,#3a5828 100%);
border-radius:var(–r);padding:14px 16px;margin:12px 16px 0;
box-shadow:var(–sh1);display:flex;align-items:center;gap:12px;
}
.notif-card-txt{flex:1;}
.notif-card-title{font-size:14px;font-weight:800;color:#fff;}
.notif-card-sub{font-size:11px;color:rgba(255,255,255,.75);margin-top:2px;font-weight:500;line-height:1.5;}
.notif-card-btn{background:rgba(255,255,255,.22);border:none;color:#fff;
border-radius:20px;padding:7px 14px;font-family:inherit;font-size:12px;
font-weight:800;cursor:pointer;white-space:nowrap;}
.notif-card-btn:active{background:rgba(255,255,255,.35);}
...
<div id="app">
  ...
  <div class="upd-banner" id="updBanner" onclick="doUpdate()">
    <span class="upd-txt">🌿 New version available</span>
    <button class="upd-btn">Update Now</button>
  </div>
  ...
  <header class="hdr">
    ...
    <div class="hdr-r">
      <div style="display:flex;align-items:center;gap:0">
        ...
        <button class="notif-btn" id="notifBtn" onclick="requestNotifPermission()" title="Enable alerts">🔔</button>
      </div>
...
<div class="notif-card" id="notifCard" style="display:none">
  ...
  <div class="notif-card-txt">
    <div class="notif-card-title">Enable Block Alerts</div>
    <div class="notif-card-sub">Get notified when each block starts and ends — even when the app is in background</div>
  </div>
  <button class="notif-card-btn" onclick="requestNotifPermission()">Allow</button>
</div>
...
<div class="inst-hero">
  <div class="inst-top-bar">
    <div class="inst-head">
      <div style="font-size:26px;line-height:1">📲</div>
      <div>
        <div class="inst-title">Install &amp; Update via GitHub Pages</div>
        <div class="inst-sub">Free forever · Offline after first open</div>
      </div>
    </div>
    <div class="inst-steps">
      <div class="istep"><div class="in">1</div><div class="it">Go to <b>github.com</b> on Safari · sign up free · create a <b>Public</b> repo named <b>focusflow</b></div></div>
      <div class="istep"><div class="in">2</div><div class="it">Tap <b>"uploading an existing file"</b> → pick <b>index.html</b> → tap <b>Commit changes</b></div></div>
      <div class="istep"><div class="in">3</div><div class="it">Tap <b>Settings → Pages → Source: main → Save</b></div></div>
      <div class="istep"><div class="in">4</div><div class="it">Open <b>yourusername.github.io/focusflow</b> in Safari → Share ⎋ → <b>Add to Home Screen</b></div></div>
      <div class="istep"><div class="in">↑</div><div class="it"><b>To update the app:</b> upload a new index.html to your GitHub repo — the green banner will appear next time you open it with internet</div></div>
    </div>
  </div>
...
<div class="s-wrap">
  <div class="ss" style="margin-top:20px">
    <div class="shd">Alerts &amp; Notifications</div>
    <div class="scard">
      <div class="srow">
        <div><label>Block start alert</label><small>Sound + notification when block begins</small></div>
        <button class="tog on" id="tAlertStart" onclick="togSet('alertStart',this)"></button>
      </div>
      <div class="srow">
        <div><label>Block end alert</label><small>Sound + notification when block ends</small></div>
        <button class="tog on" id="tAlertEnd" onclick="togSet('alertEnd',this)"></button>
      </div>
      <div class="srow">
        <div><label>Sound tone</label><small>Audio chime with each alert</small></div>
        <button class="tog on" id="tSound" onclick="togSet('sound',this)"></button>
      </div>
    </div>
  </div>
  ...
  <div class="foot">FocusFlow v2.1 · Data on your device only</div>
</div>
```
...
<script>
/* ══════════════════════════════════════
   APP VERSION — bump this string when
   you upload a new index.html to GitHub
   ══════════════════════════════════════ */
var APP_VER = '2.1';
...
/* ═══ STATE ═══ */
var P={showNow:true,showHalf:true,defDur:30,alertStart:true,alertEnd:true,sound:true};
var B={};
...
var alertTimers=[], toastTimer=null;
...
/* ═══ AUDIO CHIME ═══ */
/* Uses Web Audio API — works offline, no file needed */
function playChime(type){
  ...
  try{
    ...
    if(type==='start'){
      /* Ascending two-tone — "begin" */
      note(523,0,   0.4, 0.35);  /* C5 */
      note(659,0.18,0.5, 0.35);  /* E5 */
      note(784,0.36,0.7, 0.3);   /* G5 */
    } else {
      /* Descending gentle — "done" */
      note(784,0,   0.4, 0.3);   /* G5 */
      note(659,0.18,0.4, 0.3);   /* E5 */
      note(523,0.36,0.8, 0.25);  /* C5 */
    }
  }catch(e){}
}
...
function showToast(icon,msg,dur){
  ...
  toastTimer=setTimeout(function(){t.classList.remove('show');},dur||4000);
}

/* ═══ SYSTEM NOTIFICATION ═══ */
function sysNotif(title,body,icon){
  if(typeof Notification==='undefined')return;
  if(Notification.permission==='granted'){
    try{new Notification(title,{body:body,icon:icon||'',badge:icon||''});}catch(e){}
  }
}
...
function requestNotifPermission(){
  if(typeof Notification==='undefined'){
    showToast('ℹ️','Notifications need iOS 16.4+ and Home Screen install',4000);
    return;
  }
  if(Notification.permission==='granted'){
    showToast('✅','Alerts already enabled!',3000);
    updateNotifUI();
    return;
  }
  Notification.requestPermission(function(result){
    if(result==='granted'){
      showToast('✅','Block alerts enabled!',3000);
      scheduleAlerts();
    } else {
      showToast('ℹ️','Enable in iPhone Settings → FocusFlow → Notifications',5000);
    }
    updateNotifUI();
  });
}
...
function updateNotifUI(){
  ...
  if(nb)nb.style.opacity=granted?'1':'0.4';
  var nc=document.getElementById('notifCard');
  if(nc)nc.style.display=granted?'none':'flex';
}
...
function scheduleAlerts(){
  ...
  if(!P.alertStart&&!P.alertEnd)return;

  var now=new Date(), todayBlocks=getB(todK());
  var nowMs=now.getTime();

  todayBlocks.forEach(function(b){
    var parts=b.start.split(':'), bh=+parts[0], bm=+parts[1];
    var startMs=new Date(now.getFullYear(),now.getMonth(),now.getDate(),bh,bm,0).getTime();
    var endMs=startMs+b.dur*60000;
    var cat=CATS.find(function(c){return c.id===b.cat;})||CATS[0];

    /* Start alert */
    if(P.alertStart){
      var msToStart=startMs-nowMs;
      if(msToStart>-5000 && msToStart<86400000){
        var delay=Math.max(0,msToStart);
        alertTimers.push(setTimeout(function(block,catObj){
          return function(){
            playChime('start');
            var msg='Starting: '+block.act+' ('+mStr(block.dur)+')';
            showToast(catObj.e,msg,5000);
            sysNotif('▶ '+block.act,mStr(block.dur)+' block starting now',catObj.e);
          };
        }(b,cat),delay));
      }
    }
    ...
    /* End alert */
    if(P.alertEnd){
      var msToEnd=endMs-nowMs;
      if(msToEnd>0 && msToEnd<86400000){
        alertTimers.push(setTimeout(function(block,catObj){
          return function(){
            playChime('end');
            var msg='Done: '+block.act+' ✓';
            showToast('✅',msg,5000);
            sysNotif('✅ '+block.act+' complete','Block finished — great work!',catObj.e);
            renderDay(); /* refresh to remove active-now highlight */
          };
        }(b,cat),msToEnd));
      }
    }
...
window.onload=function(){
  ...
  scheduleAlerts();
  updateNotifUI();
  /* Show notif card if permission not yet granted */
  if(typeof Notification!=='undefined'&&Notification.permission!=='granted'){
    var nc=document.getElementById('notifCard');
    if(nc)nc.style.display='flex';
  }
  /* Reschedule alerts every minute (catches midnight rollover) */
...
function renderDay(){
  ...
  bl.forEach(function(b){
    ...
    var isActive=isT&&(sm<=nowMins&&nowMins<sm+b.dur);
    var el=document.createElement('div');
    el.className='tblk'+(isActive?' active-now':'');
...
function setupResize(handle,block,key){
  ...
  handle.addEventListener('pointerdown',function(e){
    ...
    function up(){
      ...
      scheduleAlerts(); /* reschedule after resize */
    }
...
function saveBlock(){
  ...
  scheduleAlerts(); /* always reschedule after any change */
...
function delBlock(){
  ...
  closeModal(); renderDay(); scheduleAlerts();
}
...
function initSettings(){
  ...
  document.getElementById('tAlertStart').className='tog'+(P.alertStart?' on':'');
  document.getElementById('tAlertEnd').className='tog'+(P.alertEnd?' on':'');
  document.getElementById('tSound').className='tog'+(P.sound?' on':'');
  buildScatg();
}
function buildScatg(){
...
function togSet(key,btn){
  ...
  if(key==='alertStart'||key==='alertEnd')scheduleAlerts();
  if(activeTab()==='day')renderDay();
}
function doExport(){
  try{
